...
dataset = Generator.generate_closure_dataset(
    256,
    2,
...
(i, data) = first(enumerate(dataset))
...
dataset = Generator.read_dataset(
    "./dataset/viscous_burgers_high_dim_t2_64_xpi_64_nu0.04_typ2_m10_256_up16_j173.jld2",
)["training_set"];
for (i, data) in enumerate(dataset)
    t, u, _, _ = data
    # display(GraphicTools.show_state(u, t, x, "", "t", "x"))
...
begin
    ...
    t, u₀, u = Generator.get_burgers_batch(2.0, 0.0, pi, 0.0, 64, 64, 0.04, 2, (; m = 5))
...
t, u₀, u = Generator.get_burgers_batch(
    4 * tₘₐₓ,
    tₘᵢₙ,
    4 * xₘₐₓ,
    xₘᵢₙ,
