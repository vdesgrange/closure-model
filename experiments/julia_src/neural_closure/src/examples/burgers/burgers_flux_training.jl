...
# function train(epochs, model, dataset)
#   loss_fn = Flux.mse;
#   optimizer = Flux.ADAM(0.01, (0.9, 0.999), 1.0e-8);
#
#   (t_train, t_val), (train_loader, val_loader) = split_data(dataset, 0.7, 32, false);
#   learner = FluxTraining.Learner(model, (train_loader, val_loader), optimizer, loss_fn; callbacks=[FluxTraining.Metrics(Flux.mse)], optimizer);
#
